...
Type A ex: AB12 : AB=column coordinate, 12 = rows. Type B ex: R1C2: 1 is rows, 2 is columns.*/
...
int id(string entry) {
	/*we identify an entry as type B (R#C#) by determining that firstIsR is true, scndIsLtr is false, and foundC is true. 
	return 0 = A and 1 = B*/
...
string convertToB(string orig) {
	...
	string rslt = "R";//we're coverting to B from A, we know that the first character will be R since the format is R#C#
	...
	//finPos now contains the number of letters-1 (and the index of the final letter). Use this number to control the loop that processes the letters
	int total = 0;
	int exponent = 0;
	...
	0 (ie numbers in our base 26 system are 1-26 not 0-25). When viewed in this light it's almost simple base 26 to base 10 conversion.*/
...
string convertToA(string orig) {
	...
	/*Again, we're taking a "convert between different bases" route, this time from base 10 to base 26. 
	Take as large a bite as possible out of our base 10 total using base 26 without going over. Repeat this with
	whats left over,... until our original base 10 number, colsInt, is gone.*/
	...
	//we could possibly need (since column int is > 10^6 and 26^5 = 11881376).
		
	int letter = 26; //26 = z ... 1 = a. start at z, Z*26^exponent gives us the largest possible bite for that exponent

	int charbuf; //used to test for special cases below
	...
	while (colsInt != 0) {//while the remainder!=0
		...
		while (colsInt - (pow(26.0, exponent) * letter) < 0) { //if we've taken too big a bite, take a step back. First way we can step back is by shrinking the coefficient.
			letter--;
			if (letter == 0) { //if we've tried all coefficients, its a special case. We must try a different exponent.
				break;			
			}
		}
		...
		The other special case is that we can't find a coefficient that works for the current exponent. The way to fix both these problems is the same: decrement the coefficient.
		...
		and none of them work, we decrement the coefficient tied to the previous exponent. Note that
		it is possible that the coefficient of the previous exponent is also 'A'. In this case we have to
		remove it since we can't decrement it, and move along to the next-earliest exponent. If we get all the way to exponent=4 and we encounter this problem we have no choice but to
		decrement the exponent and to start the result from the next-smallest exponent, 3.*/
		...
		if (exponent != 0 && (colsInt - (pow(26.0, exponent) * letter) == 0) || letter == 0) {
			if (letter != 0 && letter != 1) letter--; //if the letter isn't 'A', or we haven't run through all the letters, we can simply decrement the coeff
				...
				if (rslt.size() != 0){ //If the rslt string isn't empty we can backtrack. If it is empty we decrement the exponent.
					do{
						...
						if (charbuf == 64) { //remove the 'A'
							...
							colsInt += pow(26.0, exponent);//add the difference back in
						}
					} while (charbuf == 64);
		...
		/*If we're here we've found a combination that works. Record it.*/
